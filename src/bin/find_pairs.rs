...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for symbol in &candidates {
        ...
        match client
            .get_product_candles(symbol, &start, &end, Granularity::OneHour)
            .await
        {
            Ok(candles) => {
                // Sort by time
    ...
    for pair in results.iter().take(10) {
        println!(
            "Selected: {} - {} (Corr: {:.2}, Half-Life: {:.1}h)",
            pair.symbol_a, pair.symbol_b, pair.correlation, pair.mean_reversion_half_life
        );
